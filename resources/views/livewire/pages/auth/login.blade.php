...
<div>
    ...
    <div class="py-4">
        <div class="text-center text-2xl py-2">Welcome back!</div>
        <h3 class="text-center">Kindly fill in your login detail to proceed</h3>
    </div>
    <form wire:submit="login">
        <!-- Email Address -->
        <div>
            <x-input-label for="email" :value="__('Email')" />
            <x-text-input wire:model="email" id="email" class="block mt-1 w-full" place type="email" name="email" required autofocus autocomplete="username" />
        ...
        <div class="flex flex-col items-center justify-end mt-4">
            @if (Route::has('password.request'))
            <x-primary-button class="mt-3 w-1/4 text-center">
                {{ __('Log in') }}
            </x-primary-button>

            <!-- <a class="underline text-sm text-gray-600 mt-3 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100 rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 dark:focus:ring-offset-gray-800" href="{{ route('password.request') }}" wire:navigate>
                    {{ __('Forgot your password?') }}
                </a> -->
            @endif
